...
function RouteInput({ label, placeholder, value, onChange, disabled }) {
  return (
...
function RerouteBanner({ incident, onDismiss, onReroute, rerouteLoading }) {
  return (
    <div
      className="absolute top-4 left-1/2 -translate-x-1/2 z-20 flex items-center gap-3 bg-white border border-[#F5C0B8] rounded-2xl px-5 py-3.5"
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="text-[13px] font-semibold text-[#111210]">Incident on your route</div>
        <div className="text-[12px] text-[#7A7A72] truncate">
          {incident?.type || 'Incident'} — {incident?.description || 'Detected within 300m of your route.'}
        </div>
      </div>
...
function MapPageInner() {
  ...
  async function handleReroute() {
    ...
    try {
      directionsServiceRef.current.route(
        { origin: currentUserLocation, destination: originalDestination, travelMode: 'DRIVING', provideRouteAlternatives: true },
        (result, status) => {
          if (status === 'OK') {
    ...
    <div className="flex flex-1 overflow-hidden">
      ...
      <aside className="flex flex-col bg-white border-r border-[#E2E1DB] overflow-y-auto flex-shrink-0" style={{ width: '320px' }}>
        <PanelSection title="Plan your route">
          <div className="flex flex-col gap-3">
            <RouteInput label="From" placeholder="e.g. Bastos, Yaoundé" value={origin} onChange={setOrigin} disabled={predictLoading} />
