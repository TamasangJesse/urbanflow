  ...
  return (
    <div
      className={`flex items-start gap-3 md:gap-4 p-4 md:p-5 rounded-xl border transition-colors ${
        isRead ? 'bg-white border-[#E2E1DB]' : 'bg-[#F0F6FF] border-[#C5D9F5]'
      }`}
    >
      {/* Icon */}
      <div
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2">
          <p className={`text-[13px] md:text-[14px] leading-relaxed ${isRead ? 'text-[#3D3D38]' : 'text-[#111210] font-medium'}`}>
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center py-16 md:py-20 text-center">
      <div className="w-14 h-14 md:w-16 md:h-16 rounded-full bg-[#F7F6F2] border border-[#E2E1DB] flex items-center justify-center text-2xl md:text-3xl mb-4">
        🔔
      </div>
      <h3 className="text-[15px] md:text-[16px] font-semibold text-[#111210] mb-1">
      ...
      <main className="flex-1 flex justify-center py-8 md:py-12 px-4">
        <div className="w-full max-w-2xl">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6 md:mb-8">
            <div>
              <h1
                className="text-[24px] md:text-[28px] font-bold text-[#111210] mb-1"
                style={{ fontFamily: 'Georgia, serif', letterSpacing: '-0.5px' }}
              >
                Notifications
              </h1>
              <p className="text-[13px] md:text-[14px] text-[#7A7A72]">
                {unreadCount > 0
            ...
            {unreadCount > 0 && (
              <Button
                ...
                size="sm"
                onClick={markAllRead}
                className="!rounded-lg !text-[13px] self-start sm:self-auto flex-shrink-0"
              >
                Mark all as read
              </Button>
            )}
