...
def make_token(
    ...
    """
    Generate a valid signed JWT for use in test requests.
    Mirrors exactly how the User Service generates tokens.
    """
...
def make_expired_token(
    ...
    Used to verify the gateway rejects expired tokens.
    """
...
def mock_redis():
    ...
    - exists() returns 0 by default (token is NOT blacklisted)
    - ping() returns True (Redis is reachable)
    Override in individual tests to simulate blacklisted tokens or failures.
    """
    redis = AsyncMock()
...
async def client(mock_redis):
    ...
    Patches app.state.redis and app.state.http_client at startup
    so tests never touch real Redis or real downstream services.
    """
